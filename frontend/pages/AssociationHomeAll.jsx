...
export default function AssociationHomeAll({ route }) {
  ...
  const handlePostRequest = async () => {
    if (!description.trim()) {
      Alert.alert("تنبيه", "يرجى كتابة وصف للطلب أولاً.");
      return;
    }
            ...
            {!isFormVisible ? (
              <>
                ...
                <View style={styles.cardShadowWrapper}>
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate("DashboardAssociationAll", route.params)}
                  >
                    <View style={styles.imageBox}>
                      <Image source={require('../assets/images/requestall.png')} style={styles.fullImg} resizeMode="cover" />
                    </View>
                    <View style={styles.cardFooter}>
                      <Text style={styles.cardTitle}>General Donation</Text>
                ...
                {requests.map(item => (
                  <View key={item.request_id} style={styles.requestCard}>
                    <Text style={styles.reqTitle}>{item.donation_type} Request</Text>
              ...
              <View style={styles.formCard}>
                <View style={styles.formHeader}>
                  <Text style={styles.formTitle}>New General Request</Text>
                ...
                <View style={styles.typeSelector}>
                  {['FOOD', 'CLOTHES'].map((type) => (
                    <TouchableOpacity 
                      key={type} 
                      style={[styles.typeBtn, donationType === type && styles.activeTypeBtn]}
                      onPress={() => setDonationType(type)}
                    >
                      <Text style={[styles.typeBtnText, donationType === type && styles.activeTypeText]}>{type}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                <TouchableOpacity style={styles.postBtn} onPress={handlePostRequest}>
                  <Text style={styles.postBtnText}>Post to ZAAD</Text>
...
const styles = StyleSheet.create({
  ...
  typeBtn: { paddingVertical: 10, paddingHorizontal: 15, borderRadius: 12, borderWidth: 1, borderColor: '#EBE1D7', backgroundColor: '#f9f9f9' },
  activeTypeBtn: { backgroundColor: '#A27571', borderColor: '#A27571' },
  typeBtnText: { fontSize: 12, color: '#54403c', fontWeight: 'bold' },
  activeTypeText: { color: '#fff' },

  inputArea: { backgroundColor: '#f9f9f9', borderRadius: 20, padding: 15, height: 120, textAlignVertical: 'top', borderWidth: 1, borderColor: '#EBE1D7', fontSize: 16, color: '#54403c', marginBottom: 20 },
