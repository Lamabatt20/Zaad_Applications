...
export default function RejectedClothesScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("[FETCH] GET", `${config.API_URL}/donations/clothes/rejected`);
      const res = await axios.get(`${config.API_URL}/donations/clothes/rejected`);
      const arr = Array.isArray(res.data) ? res.data : res.data?.data ?? [];
      setItems(arr.map(normalizeDonation));
    } catch (e) {
      setErrorMsg(typeof e?.message === "string" ? e.message : "Failed to load rejected donations");
      setItems([]);
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={{ textAlign: "center", marginTop: 30 }}>No rejected donations</Text>}
